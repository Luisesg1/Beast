...
function RadarChart({ data }) {
  ...
  return (
    <svg width="100%" viewBox="0 0 200 210" style={{ maxWidth: 260, margin: "0 auto", display: "block" }}>
      ...
      {/* Data polygon */}
      <polygon points={polyPoints} fill="rgba(232,255,0,0.12)" stroke="#e8ff00" strokeWidth="2" strokeLinejoin="round" />
      {/* Data points */}
      {points.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="3.5" fill="#e8ff00" />
      ))}
      {/* Labels */}
      {points.map((p, i) => (
        <text key={i} x={p.lx} y={p.ly} textAnchor="middle" dominantBaseline="middle"
          fill={p.pct > 0 ? "#e8ff00" : "rgba(255,255,255,0.3)"}
...
// ─── Mini Line Chart SVG ──────────────────────────────────────────────────────
function LineChart({ data, color = "#e8ff00", height = 80 }) {
  ...
  return (
    <div>
            ...
            <div key={wi} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {week.map((cell, di) => (
                <div key={di} title={cell.dateStr} style={{
                  ...
                  background: cell.isFuture ? "transparent" : cell.trained ? "#e8ff00" : "rgba(255,255,255,0.06)",
                  border: cell.isFuture ? "none" : `1px solid ${cell.trained ? "#e8ff00" : "rgba(255,255,255,0.04)"}`,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
        ...
        <div style={{ width: 10, height: 10, borderRadius: 2, background: "#e8ff00", marginLeft: 8 }} />
...
function GoalsSection({ sessions, uid, onPickExercise, isPro = false }) {
  ...
  if (!isPro) return (
    <div style={{ textAlign: "center", padding: "32px 16px" }}>
      ...
      <div style={{ background: "rgba(232,255,0,0.05)", border: "1px solid rgba(232,255,0,0.15)", borderRadius: 14, padding: "16px", marginBottom: 20, textAlign: "left" }}>
  ...
  return (
    <div>
      ...
      {goalsWithCurrent.map(g => {
        ...
        const done = pct >= 100;
        return (
          <div key={g.id} style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${done ? "rgba(232,255,0,0.4)" : "rgba(255,255,255,0.08)"}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              <div>
                <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 16, fontWeight: 800 }}>{g.exercise}</div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>
                  Meta: <strong style={{ color: done ? "#e8ff00" : "rgba(255,255,255,0.7)" }}>{g.target} {g.unit} 1RM</strong>
                  {" · "}Actual: <strong style={{ color: "#e8ff00" }}>{g.current} {g.unit}</strong>
                </div>
              </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 6, height: 8, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${pct}%`, background: done ? "#e8ff00" : "linear-gradient(90deg, rgba(232,255,0,0.6), #e8ff00)", borderRadius: 6, transition: "width 0.6s ease" }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>0</span>
              <span style={{ fontSize: 10, color: done ? "#e8ff00" : "rgba(255,255,255,0.5)", fontWeight: 700 }}>{Math.round(pct)}%</span>
      ...
      {adding ? (
        <div style={{ background: "rgba(232,255,0,0.04)", border: "1px solid rgba(232,255,0,0.2)", borderRadius: 14, padding: 16, marginBottom: 10 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#e8ff00", marginBottom: 12, letterSpacing: 1, textTransform: "uppercase" }}>Nueva meta</div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={addGoal} style={{ flex: 1, background: "#e8ff00", color: "#000", border: "none", borderRadius: 8, padding: "10px", fontWeight: 800, fontSize: 13, cursor: "pointer", fontFamily: "Barlow Condensed, sans-serif", letterSpacing: 1 }}>GUARDAR</button>
        ...
        <button onClick={() => setAdding(true)} style={{ width: "100%", background: "rgba(232,255,0,0.07)", border: "1px dashed rgba(232,255,0,0.3)", borderRadius: 12, padding: "12px", color: "#e8ff00", fontWeight: 800, fontSize: 13, cursor: "pointer", fontFamily: "Barlow Condensed, sans-serif", letterSpacing: 1 }}>
          + AGREGAR META
        </button>
      )}
    </div>
    ...
    return (
      <div className="overlay" onClick={onClose}>
        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 340, textAlign: "center", padding: "32px 24px" }}>
          ...
          <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 22, fontWeight: 900, color: "#e8ff00", letterSpacing: 2, marginBottom: 6 }}>
            ESTADÍSTICAS PRO
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: watchingAd ? "rgba(255,255,255,0.1)" : "rgba(232,255,0,0.12)",
              border: "1px solid rgba(232,255,0,0.4)",
              color: watchingAd ? "rgba(255,255,255,0.4)" : "#e8ff00",
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal modal-wide" onClick={e => e.stopPropagation()} style={{ maxHeight: "90vh", overflowY: "auto" }}>
        ...
        {/* Header */}
        <div className="modal-header" style={{ borderBottom: "1px solid rgba(232,255,0,0.15)", paddingBottom: 14, marginBottom: 0 }}>
          <div>
            <h3 className="modal-title" style={{ margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ background: "rgba(232,255,0,0.12)", border: "1px solid rgba(232,255,0,0.3)", borderRadius: 8, padding: "2px 8px", fontSize: 11, fontWeight: 800, color: "#e8ff00", letterSpacing: 2, fontFamily: "Barlow Condensed, sans-serif" }}>PRO</span>
              Estadísticas Avanzadas
            </h3>
            {!isPro && unlockUntil && timeLeft && (
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>
                ⏱ Acceso temporal: <span style={{ color: "#e8ff00", fontWeight: 700 }}>{timeLeft}</span>
              </div>
            )}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, padding: "14px 0 10px" }}>
          {[
            ...
            <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
              ...
              <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 18, fontWeight: 800, color: "#e8ff00" }}>{s.value}</div>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: 0 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              flex: 1, background: "none", border: "none", borderBottom: `2px solid ${tab === t.id ? "#e8ff00" : "transparent"}`,
              color: tab === t.id ? "#e8ff00" : "rgba(255,255,255,0.35)",
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 16, justifyContent: "center" }}>
              {[
                ...
                <button key={p.v} onClick={() => setRadarPeriod(p.v)} style={{
                  background: radarPeriod === p.v ? "rgba(232,255,0,0.12)" : "rgba(255,255,255,0.04)",
                  border: `1px solid ${radarPeriod === p.v ? "rgba(232,255,0,0.4)" : "rgba(255,255,255,0.08)"}`,
                  color: radarPeriod === p.v ? "#e8ff00" : "rgba(255,255,255,0.4)",
                    ...
                    return (
                      <div key={muscle} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        ...
                        <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 4, height: 6, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${pct}%`, background: "#e8ff00", borderRadius: 4 }} />
                        </div>
                        <div style={{ width: 32, fontSize: 11, color: "#e8ff00", fontWeight: 800, textAlign: "right" }}>{pct}%</div>
                      </div>
                    );
                  })}
                </div>
            ...
            {lineData.length < 2 ? (
              ...
              <>
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "16px 12px", marginBottom: 16 }}>
                  ...
                  <LineChart data={lineData} color="#e8ff00" height={100} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "10px 8px", textAlign: "center" }}>
                      <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 20, fontWeight: 800, color: "#e8ff00" }}>{s.value}</div>
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  {exerciseHistory.slice(-8).reverse().map((h, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: 13 }}>
                      ...
                      <span style={{ fontWeight: 800, color: "#e8ff00" }}>{h.rm} kg 1RM</span>
                    </div>
                  ))}
                </div>
              </>
            ...
            {(() => {
              ...
              return (
                <div style={{ marginBottom: 24 }}>
                      ...
                      return (
                        <div key={m.label} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "14px 16px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 4 }}>
                            <span style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 32, fontWeight: 900, color: "#e8ff00", lineHeight: 1 }}>{m.fmt(m.this)}</span>
                            {!neutral && (
                  ...
                  {thisPRs.length > 0 && (
                    <div style={{ background: "rgba(232,255,0,0.06)", border: "1px solid rgba(232,255,0,0.2)", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
                      🏆 <strong style={{ color: "#e8ff00" }}>{thisPRs.length} PR{thisPRs.length > 1 ? "s" : ""}</strong> esta semana: {thisPRs.slice(0, 3).join(", ")}{thisPRs.length > 3 ? ` +${thisPRs.length - 3} más` : ""}
                    </div>
                  )}
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    ...
                    <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 4, height: 8, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${Math.min(pct, 100)}%`, background: count >= 12 ? "#e8ff00" : count >= 6 ? "#3b82f6" : "#f97316", borderRadius: 4 }} />
                    </div>
